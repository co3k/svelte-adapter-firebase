...
const entrypoint = function (options = {}) {
  return {
    ...
    async adapt(builder) {
      ...
      const { functions, publicDir } = parseFirebaseConfiguration({
        firebaseJsonPath,
        target,
        sourceRewriteMatch,
      });
      ...
      ensureStaticResourceDirsDiffer({
        source: path.join(process.cwd(), builder.getStaticDirectory()),
        dest: publicDir,
      });

      const functionsPackageJson = JSON.parse(
        readFileSync(path.join(functions.source, "package.json"), "utf-8")
      );
      if (!functionsPackageJson?.main) {
        throw new Error(
          `Error reading ${functionsPackageJson}. Required field "main" missing.`
        );
      }
      ...
      builder.copy(
        path.join(dirs.files, "firebase-to-svelte-kit.js"),
        path.join(dirs.tmp, "firebase-to-svelte-kit.js")
      );
      ...
      try {
        if (
          !readFileSync(ssrFunc.entrypoint, "utf-8").includes(
            `${functions.name} =`
          )
        ) {
          builder.log.warn(
            `Add the following Cloud Function to ${ssrFunc.entrypoint}`
          );
          builder.log.warn(`
...
exports.${functions.name} = functions.region("us-central1").https.onRequest(async (request, response) => {
	...
	return ${ssrFunc.svelteSSR}(request, response);
        ...
        throw new Error(
          `Error reading Cloud Function entrypoint file: ${ssrFunc.entrypoint}. ${error.message}`
        );
      }

      builder.log.minor(
        logRelativeDir(
          "Erasing destination static asset dir before processing",
          publicDir
        )
      );
      ...
      builder.log.minor(
        logRelativeDir("Writing client application to", publicDir)
      );
      builder.writeStatic(publicDir);
      builder.writeClient(publicDir);

      builder.log.minor(
        logRelativeDir("Prerendering static pages to", publicDir)
      );
